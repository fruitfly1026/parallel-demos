/* ============================================================================
   ...
   ----------------------------------------------------------------------------
   PROVENANCE: see smem_abuse/smem_abuse_naive.cu.  Self-contained kernel with
   PASTA's shape, not PASTA's code.

   ############################################################################
   #  WHAT CHANGED vs smem_abuse/smem_abuse_naive.cu                          #
   ############################################################################
   ...
   Build: nvcc -O3 -arch=sm_86 -lineinfo -o smem_abuse_opt smem_abuse_opt.cu
   ========================================================================= */
#include <cstdio>
#include <cstdlib>
#include <cmath>
...
int main(void) {
    ...
    printf("\nTiming: %.4f ms\n", ms);
    printf("\nCompare with smem_abuse/smem_abuse_naive.cu (same math via shared memory).\n");
...
Timing: 0.0662 ms      vs smem_abuse_naive.cu 0.0745 ms = 1.13x.
